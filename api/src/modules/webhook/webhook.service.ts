...
export class WebhookService {
  ...
  async processWebhook(payload: WebhookDto): Promise<void> {
    ...
    if (project) {
      ...
      this.logger.log(
        `Updated project ${projectId} status to ${payload.status}`
      );
      ...
      if (payload.status === "completed") {
        try {
         ...
         this.logger.debug(`Using ${mediaItems.length} media items`);
          ...
          this.logger.log(
            `Media matching analysis completed for project ${projectId}. Found ${analysisResult.totalMatches} matches`
          );
          ...
          this.logger.error(
            `Error processing OpenAI media matching for project ${projectId}:`,
            error
          );
  ...
  private updateBatchStatus(
    ...
    this.logger.log(
      `Updating batch ${batchId} for project ${projectId} with status ${status}`
    );
    ...
    if (batch) {
      ...
      if (finishedProjects >= totalProjects) {
        ...
        this.logger.log(
          `Batch ${batchId} completed: ${completedProjects} successful, ${failedProjects} failed`
        );
      }
    }
  }
